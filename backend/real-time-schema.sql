-- Real-Time Features Database Schema
...
BEGIN
    ...
    RETURN distance <= (geofence_record.radius / 1000); -- Convert radius from meters to kilometers
END;
$$ LANGUAGE plpgsql;
...
SELECT 
    ...
    sc.user_role,
    sc.connection_time,
    EXTRACT(EPOCH FROM (NOW() - sc.connection_time)) as session_duration_seconds
FROM socket_connections sc
WHERE sc.disconnection_time IS NULL;
...
UNION ALL
SELECT 
    'active_rides' as metric_name,
    COUNT(*) as metric_value,
    'count' as metric_unit,
    'business' as category,
    NOW() as timestamp
FROM rides 
WHERE status IN ('accepted', 'in_progress', 'pickup');
...
INSERT INTO geofences (name, type, center_lat, center_lng, radius, properties) VALUES
('Downtown Pickup Zone', 'pickup_zone', 40.7128, -74.0060, 500, '{"message": "Welcome to downtown pickup zone"}'),
('Airport Dropoff Zone', 'dropoff_zone', 40.6413, -73.7781, 300, '{"message": "Airport dropoff zone"}'),
('School Zone Speed Limit', 'speed_limit_zone', 40.7589, -73.9851, 200, '{"speedLimit": 20, "message": "School zone - reduce speed"}'),
('Restricted Area', 'restricted_area', 40.7505, -73.9934, 150, '{"message": "Restricted area - no pickup allowed"}')
ON CONFLICT DO NOTHING;

-- Insert sample analytics metrics
INSERT INTO analytics_metrics (metric_name, metric_value, metric_unit, category) VALUES
('active_drivers', 0, 'count', 'business'),
('active_riders', 0, 'count', 'business'),
('average_wait_time', 0, 'minutes', 'performance'),
('completion_rate', 0, 'percentage', 'business'),
('system_health', 100, 'percentage', 'system')
ON CONFLICT DO NOTHING;
